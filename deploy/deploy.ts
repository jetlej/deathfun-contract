...
export default async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (localNetworkNames.includes(hre.network.name)) {
    ...
    const richWalletSigner = await provider.getSigner(
      LOCAL_RICH_WALLETS[0].address
    );
    ...
    if (parseFloat(balanceEther) < requiredEther) {
      ...
      console.log(
        `Deployer account funded. New balance: ${ethers.formatEther(
          newBalanceWei
        )} ETH`
      );
    } else {
  ...
  try {
    ...
    console.log(
      `Balance right before deploy: ${ethers.formatEther(balanceWei)} ETH`
    );
    if (balanceWei === 0n) {
      // Use strict check for 0n
      console.error(
        "Deployer account still has zero balance before deployment attempt!"
      );
  ...
  // Deploy the contract
  console.log(
    `Deploying ${artifact.contractName} with account: ${deployer.zkWallet.address}...`
  );
  try {
  ...
  if (contractAddress) {
    ...
    if (hre.network.name === "abstractTestnet") {
      ...
      console.log(
        `(Verification might not be configured for network: ${hre.network.name})`
      );
    }
    console.log("---");
  } else {
